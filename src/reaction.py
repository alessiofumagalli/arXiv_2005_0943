...
class Reaction(object):
    def __init__(self, **kwargs):
        # set the name of the solute and precipitate
        self.solute = kwargs["solute"]
        self.precipitate = kwargs["precipitate"]

        # dimensionless parameter
        ref = kwargs["reference"]
        self.adim = ref["length"]/ref["gamma_eq"]/ref["velocity"]

        self.theta = ref["theta"]
        self.reaction = kwargs["reaction"]

    def step(self, uw_0, deltaT, tol=1e-12, max_it=1e2):
        ...
        neg_n = pos_1[np.where(uw_n[:, pos_1] < 0)[0]]

        for i in neg_n:
            # initialization of the bisection algorithm
            err, it = np.abs(uw_n[1, i]), 0
