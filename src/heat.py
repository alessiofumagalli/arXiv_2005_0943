...
class Heat(object):
    ...
    def set_data(self, data, data_time):
        ...
        for g, d in self.gb:
            ...
            if g.dim < self.gb.dim_max():
                diff = data["l_w"] * d[pp.STATE]["aperture"]
            else:
    ...
    def update_data(self):
        ...
        for g, d in self.gb:
            ...
            # assign permeability
            if g.dim < self.gb.dim_max():
                l = self.data["l_w"] * d[pp.STATE]["aperture_star"]
            else:
    ...
    def set_flux(self, flux_name, mortar_name):
        for _, d in self.gb:
            d[pp.PARAMETERS][self.adv_name][self.flux] = self.data["rc_w"] * d[pp.STATE][flux_name]

        for _, d in self.gb.edges():
            d[pp.PARAMETERS][self.adv_name][self.flux] = self.data["rc_w"] * d[pp.STATE][mortar_name]
